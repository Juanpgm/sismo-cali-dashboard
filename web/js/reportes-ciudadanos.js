...
// way as a malformed value — never crashes, never misclassifies). Shared by
// the 'panel' colour mode below and the panel pill in listItemHtml.
function panelColor(r) {
  return (r.panel && r.panel.visitado && r.panel.sticker) ? COLORS.status.h
...
export const COLOR_MODES_REPORTES = {
  ...
  panel: { label: 'Panel', colorOf: panelColor },
};
  ...
  return `<li>
    <button type="button" class="eval-row" data-rep-detail="${escapeHtml(r.id)}">
      ...
      <span class="eval-pill-group">
        ...
        ${r.panel && r.panel.visitado ? `<span class="eval-pill" style="--eval-pill:${panelColor(r)}">panel ${r.panel.sticker ? 'con sticker' : 'sin sticker'}</span>` : ''}
      </span>
