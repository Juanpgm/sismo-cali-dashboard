 ...
 *  misreport "no stickers" as fact instead of "unknown". */
function rowHtml(r, stickersLoaded) {
...
// initSeguimiento on every store notify (~3 times per 15-min auto-refresh
// cycle: setStickerCoverage, setStickerIds, applyFilters) would wipe the
...
export function initSeguimiento(root, { getToken, records }) {
  ...
  function renderTable(rows) {
    ...
    tbody.innerHTML = sorted.length
      ? sorted.map((r) => rowHtml(r, stickersLoaded)).join('')
      : `<tr><td colspan="${COLUMNS.length}" class="eval-empty">Ningún profesional coincide con los filtros aplicados.</td></tr>`;
  }
