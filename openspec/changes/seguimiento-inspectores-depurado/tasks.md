...
**Second documented divergence found during the parity fixture (2.27), beyond the permitted D7 fix**: the golden `outputs/inspectores_depurado_seguimiento.xlsx` snapshot has 3 rows with `estado_sugerido="revisar"` that are absent from BOTH Vercel and Fase 2 (confirmed against the raw `context/EDA_HAMON/01_Insumos/` source files) — per `spec.md`'s literal "Estado Sugerido Precedence" rule, absence from both sources means `"candidato_desactivacion"`, not `"revisar"`. All 3 share the base CSV's own `activo="true"` flag, a signal `explore.md`'s Q3 citation never mentioned and `spec.md` never adopted. This implementation follows `spec.md` literally (candidato_desactivacion); flagged here for a follow-up spec correction pass, same treatment as the Phase 1 blob-bundle conflict noted below. See `test_inspectores_depuracion_parity.py`'s module docstring for full tracing.
