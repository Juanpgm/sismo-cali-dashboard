...
- C. `DepuracionCache` key = `(snapshot_version, roster version, survey version, referencia huella, hoy)` with an in-flight marker so N requests produce exactly 1 compute.
...
1. Firestore budget for this path: <= 10k reads/day (20% of the Spark cap); measure E, I, S again at parity time.
2. Freshness: roster 30 min, evaluaciones 15 min, survey names 60 min, API walk 5 min; admin writes invalidate immediately.
...
- Firestore reads: <= 10k/day; with static inputs per hour <= 2 roster scans, <= 1 survey scan, <= 1-2 evaluaciones scans.
