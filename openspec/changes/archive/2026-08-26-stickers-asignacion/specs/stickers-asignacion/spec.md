...
Change: `stickers-asignacion` · New capability (no prior spec exists for this domain).
...
assign/reassign inspector control (populated from the existing `inspectores` roster already
loaded by the Stickers tab, with no new roster fetch) that calls `asignarInspector` or
`reasignarPunto`.
...
#### Scenario: Inspector dropdown reuses the existing roster
- GIVEN the Stickers tab has already loaded the `inspectores` roster for its roster section
- WHEN the Asignación sub-section renders an inspector `<select>`
- THEN it is populated from that already-loaded roster, without issuing a new roster fetch

### Requirement: Mounted as a sub-section of the existing Stickers tab
The system MUST mount the Asignación view as a sub-section inside `#view-stickers` (a segmented
control alongside Roster and Evaluaciones), and MUST NOT add a new top-level `.view-tabs` entry.
The sub-section's frontend module MUST lazy-initialize on the first time the Asignación segment is
opened, not on Stickers-tab open.
...
- GIVEN an admin opens the Stickers tab and stays on the Roster segment
- WHEN the admin has not yet opened the Asignación segment
- THEN `initStickersAsignacion` has not run and no `listPuntos`/`listCuadrillas` calls have been made
...
access). The system MUST NOT add inspector-roster CRUD (create/edit/delete inspector accounts) —
the Asignación sub-section only reads the existing `inspectores` roster to populate assignment
controls. The system MUST NOT open a public Firestore read rule for `sticker_matches` or
`cuadrillas` — both collections are reachable only through `api/sticker-asignaciones.js`
(admin-SDK), never via a client-direct Firestore read.
...
- GIVEN the Asignación sub-section's UI
- WHEN the admin looks for a way to create, edit, or delete an inspector account
- THEN no such control exists in this sub-section (inspector CRUD remains exclusively in the
  existing Roster segment)
