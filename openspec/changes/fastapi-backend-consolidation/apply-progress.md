...
## Note on branch base (this batch)

This batch's apply-progress.md continues from **Batch 3** above (the last state committed to `main` at
the time this branch was cut), NOT from Batch 4. Slice 4 (`sticker-status`/`source-status`,
`feat/fastapi-consolidation-4-status`, 104/104 backend tests green there) is a separate completed
batch that has **not been merged to `main` yet** — its own apply-progress.md sections (including the
"Cutover status sync" note recording task 1.4's completion and slice 3's 3.6/3.7 cutover) live only on
that branch. Per this batch's explicit instructions, Slice 5 depends only on Phase 1 (auth, credentials,
CORS, `create_app()`), which IS merged to `main` — so this batch branches from `main` directly rather
than stacking on the unmerged slice-4 branch. Task 1.4 (Railway "web" service) is in fact DONE in
reality (confirmed live at `sismo-cali-dashboard-production.up.railway.app`, per commit history on
`main` itself — `c2fb564`/`7dacbde`) even though `main`'s own `tasks.md`/`apply-progress.md` copies
were last updated by the slice-1b batch before that operator step completed; this batch's own
`tasks.md` edits (5.1-5.5) do not re-litigate that already-settled fact.

---

## Batch 5 — `inspector-asignaciones` (5.1-5.3 COMPLETE; 5.4 prep done, BLOCKED; 5.5 BLOCKED on 5.4)

Branch: `feat/fastapi-consolidation-5-inspector-asignaciones` (off `main`, not pushed, NOT off the
unmerged `feat/fastapi-consolidation-4-status` branch — see "Note on branch base" above). Confirmed
`python -m pytest backend/tests/ -q` on `main` before branching → **97 passed** (slice 1+2+3 only;
slice 4's routers are not present on `main`).
...
Slice 5 cannot fully close without a live `FIREBASE_ID_TOKEN` — the same class of blocker slice 2's 2.3
and slice 4's 4.5 already carry. Once a token belonging to a registered inspector is available: (1) run
`backend/scripts/verify_inspector_asignaciones_parity.py` against the real Railway URL for 5.4; (2) if
parity holds, apply 5.5's two-part repoint (`DASHBOARD_API` host flip + drop the `/api` segment in
`asignacionesApi()`'s template literal) in one commit + manual Vercel redeploy of `formulario/`. Slice 6
(`refresh` endpoint, tasks 6.1-6.4, `~130-170` lines, low 400-line risk, single PR) can start once this
branch (or slice 4's still-unmerged branch) lands on `main` — it depends only on Phase 1, already
merged. Also note: slice 4 (`sticker-status`/`source-status`) remains unmerged to `main` as of this
batch — reconciling the branch order (4 vs 5, both depend only on Phase 1 and neither depends on the
other) is an operator/maintainer decision outside this batch's scope.
