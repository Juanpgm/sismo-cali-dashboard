...
#### Scenario: Fresh EDAN-F3 snapshot
- GIVEN `meta.json` has a `generated_at` within the freshness threshold
- WHEN the EDAN-F3 row renders
- THEN its status is verde / `conectado`

#### Scenario: Stale EDAN-F3 snapshot
- GIVEN `meta.json` has a `generated_at` older than the freshness threshold
- WHEN the EDAN-F3 row renders
- THEN its status is amarillo / `desactualizado`

#### Scenario: Missing meta.json
- GIVEN `meta.json` cannot be read
- WHEN the EDAN-F3 (or Survey123) row renders
- THEN its status is amarillo / `sin metadata`

#### Scenario: Geocoding cache always shows sin metadata
- GIVEN the Geocoding source has no freshness metadata by design
- WHEN its row renders
- THEN its status is amarillo / `sin metadata` regardless of cache file age

#### Scenario: Global run failed
- GIVEN `_status.json.ok === false`
- WHEN the global pipeline run row renders
- THEN its status is rojo / `con errores`, labeled as whole-run granularity

#### Scenario: Orphaned output row
- GIVEN `asignaciones.json` exists in Blob with a last-modified timestamp
- WHEN its row renders
- THEN its status is amarillo / `sin consumidor`, and última lectura shows the last-modified
  timestamp, independent of how fresh that timestamp is

#### Scenario: Live probe succeeds independent of snapshot staleness
- GIVEN `reportes_meta.json` is stale (would be amarillo under snapshot rules)
- AND the live probe to `api/source-status.js` returns `ok: true`
- WHEN the atencionsismo row renders
- THEN the row's connectivity status is verde / `conectado`, reflecting the live probe, while the
  snapshot freshness MAY still be surfaced separately as stale

#### Scenario: Live probe reports the upstream API is down
- GIVEN `api/source-status.js` responds successfully with body `{ ok: false }`
- WHEN the atencionsismo row renders
- THEN its connectivity status is rojo / `con errores`

#### Scenario: Request to the live-probe endpoint itself fails
- GIVEN a request to `api/source-status.js` fails at the transport level (network error, or a
  non-2xx response received by the frontend caller before any `{ ok }` body, e.g. an expired
  session)
- WHEN the atencionsismo row renders
- THEN its connectivity status is amarillo / `sin datos`, NOT rojo, since the analista tab could
  not determine the upstream API's real state

#### Scenario: A source's underlying fetch fails at the transport level
- GIVEN any source's underlying `fetchData()` call rejects (network error, Blob unreachable) before
  any response body is available
- WHEN that row renders
- THEN its status is amarillo / `sin datos`, and it is NOT reported as rojo / `con errores`
...
- GIVEN a request to `api/source-status.js` with no Authorization header
- WHEN the endpoint handles the request
- THEN it responds with HTTP 401 and no probe is attempted

#### Scenario: Invalid token rejected
- GIVEN a request with a malformed or expired Firebase ID token
- WHEN the endpoint handles the request
- THEN it responds with HTTP 401 and no probe is attempted

#### Scenario: Non-admin authenticated caller rejected
- GIVEN a request with a valid token for a viewer or inspector role
- WHEN the endpoint handles the request
- THEN it responds with HTTP 403 and no probe is attempted

#### Scenario: Admin request triggers real probe and returns ok:true
- GIVEN a request with a valid admin token and a correctly configured `VISITADOS_API_PASS`
- WHEN the endpoint handles the request and the upstream API responds within the probe's
  distinguishing codes for "alive" (200 / 413 / 504)
- THEN the endpoint responds with `ok: true`, `status: 'conectado'`, and a `checked_at` timestamp

#### Scenario: Admin request with bad credentials returns ok:false
- GIVEN a request with a valid admin token but a missing or invalid `VISITADOS_API_PASS`
- WHEN the endpoint handles the request and the upstream API responds with a credential/maintenance
  failure (401 / 503) or no response
- THEN the endpoint responds with `ok: false`, `status: 'con errores'`, and a `checked_at` timestamp
...
- Modifying `refresh_data.py`'s error handling or per-source instrumentation for any source other
  than what is needed for the atencionsismo live probe.
- Building monitoring, health checks, or heartbeats for the `integracion_F1` / `cruce-gestion`
  pipeline that produces the four orphaned outputs.
- Fixing the stale `README.md` pipeline documentation.
- Adding a structured per-source `{source, ok, error}` array to `_status.json` (the global flag
  remains coarse-grained in v1).
- Adding any new reachability probe for the Google Sheet (EDAN-F3), Survey123 ArcGIS
  FeatureServer, or Google Maps Geocoding sources — these remain snapshot-freshness-only or
  "sin metadata", with no live check.

#### Scenario: Verification does not flag deferred per-source status
- GIVEN `_status.json` only exposes a whole-run `ok` flag
- WHEN this change is verified against its spec
- THEN the absence of a structured per-source `{source, ok, error}` array is NOT treated as an
  unmet requirement
